...
Physical cues: civic facade, doorway/steps, notice/community board, fountain/plaza relationship.
...
Physical cues: shopfront, awning/windows, shelves/crates/counter.
...
Physical cues: recognisable furniture displays/showroom/store counter.
...
Physical cues: parcels, sorting shelves, depot counter, route/vehicle props.
...
Physical cues: tools, workbench, vehicle silhouettes, parts storage.
...
The village is compact. A player should be able to reach the centre from any resident neighbourhood without a long empty commute. Exact travel times are measured in Studio/device playtests, but civic destinations must remain visually discoverable and ordinary traversal must not require teleport menus.
...
- two-client visit/trade behaviour;
- labels-off recognition of civic destination categories.
